...
pub enum SyncMode {
    /// Live symlink: edits in the target are immediately visible in source.
    Symlink,
    /// Full copy: target is an independent snapshot of source.
    Copy,
    /// Generated file: target is produced by transforming source into an
    /// agent-specific format (Codex .toml, Copilot .agent.md, …).
    /// Freshness is gated on a SHA-256 hash of the rendered bytes, not on
    /// the symlink identity — see `is_target_current`.
    Render,
    /// Tracked drop: a symlink (on platforms that support it) pointing at
    /// the source, distinct from `Symlink` so a later activation stage can
    /// recognise "present but not yet registered".
    Place,
}
...
impl SyncMode {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            SyncMode::Copy => "copy",
            SyncMode::Render => "render",
            SyncMode::Place => "place",
        }
    }
}
...
pub fn sync_mode_for_tool(_tool_key: &str, configured_mode: Option<&str>) -> SyncMode {
    match configured_mode {
        ...
        Some("render") => SyncMode::Render,
        Some("place") => SyncMode::Place,
        _ => SyncMode::Symlink,
    }
}
...
pub fn sync_skill(source: &Path, target: &Path, mode: SyncMode) -> Result<SyncMode> {
    ...
    match mode {
        ...
        // Render: the caller is responsible for writing the generated bytes;
        // sync_skill does not know the rendered content, so this arm is a
        // no-op stub.  The real write path calls `write_rendered_file`
        // directly and records the rendered hash as `source_hash`.
        SyncMode::Render => Ok(SyncMode::Render),
        // Place: same mechanism as Symlink — a live symlink so edits stay
        // visible — but a distinct variant so activation stages can
        // distinguish "placed" from "fully registered".
        SyncMode::Place => {
            #[cfg(unix)]
            {
                std::os::unix::fs::symlink(source, target).with_context(|| {
                    format!("Failed to create place symlink {:?} -> {:?}", target, source)
                })?;
                Ok(SyncMode::Place)
            }
            #[cfg(windows)]
            {
                match std::os::windows::fs::symlink_dir(source, target) {
                    Ok(()) => Ok(SyncMode::Place),
                    Err(_) => match junction::create(source, target) {
                        Ok(()) => Ok(SyncMode::Place),
                        Err(_) => {
                            copy_dir_recursive(source, target)?;
                            Ok(SyncMode::Copy)
...
pub fn is_target_current(
    ...
    match mode {
        ...
        // Render: target is a generated file; freshness is based on a hash
        // of the rendered bytes stored in `last_synced_source_hash` vs the
        // hash of the bytes we would render now (`current_source_hash`).
        // The target file must also exist on disk.
        SyncMode::Render => match (last_synced_source_hash, current_source_hash) {
            (Some(stored), Some(current)) if stored == current => {
                std::fs::symlink_metadata(target).is_ok()
            }
            _ => false,
        },
        // Place: a tracked symlink; current when it points at source.
        SyncMode::Place => symlink_points_to(target, source),
    }
}
...
mod tests {
    ...
    // ── SyncMode new variants — as_str / from sync_mode_for_tool ──

    #[test]
    fn sync_mode_render_as_str() {
        assert_eq!(SyncMode::Render.as_str(), "render");
    }

    #[test]
    fn sync_mode_place_as_str() {
        assert_eq!(SyncMode::Place.as_str(), "place");
    }

    #[test]
    fn sync_mode_for_tool_parses_render() {
        assert!(matches!(
            sync_mode_for_tool("codex", Some("render")),
            SyncMode::Render
        ));
    }

    #[test]
    fn sync_mode_for_tool_parses_place() {
        assert!(matches!(
            sync_mode_for_tool("some-tool", Some("place")),
            SyncMode::Place
        ));
    }

    // ── is_target_current for Render mode ──

    #[test]
    fn is_target_current_render_skips_when_hashes_match_and_target_exists() {
        let tmp = tempdir().unwrap();
        let src = tmp.path().join("source");
        let tgt = tmp.path().join("out.toml");
        fs::write(&tgt, "content").unwrap();
        assert!(is_target_current(
            &src,
            &tgt,
            SyncMode::Render,
    ...
    #[test]
    fn is_target_current_render_rerenders_when_target_missing() {
        let tmp = tempdir().unwrap();
        let src = tmp.path().join("source");
        let tgt = tmp.path().join("out.toml"); // not created
        assert!(!is_target_current(
            &src,
            &tgt,
            SyncMode::Render,
    ...
    #[test]
    fn is_target_current_render_rerenders_when_hashes_differ() {
        let tmp = tempdir().unwrap();
        let src = tmp.path().join("source");
        let tgt = tmp.path().join("out.toml");
        fs::write(&tgt, "old content").unwrap();
        assert!(!is_target_current(
            &src,
            &tgt,
            SyncMode::Render,
    ...
    // ── Place mode — sync_skill creates a symlink (unix only) ──

    #[cfg(unix)]
    #[test]
    fn sync_skill_place_creates_symlink() {
        let tmp = tempdir().unwrap();
        let src = tmp.path().join("source-dir");
        fs::create_dir_all(&src).unwrap();
        fs::write(src.join("SKILL.md"), "# hello").unwrap();
        let tgt = tmp.path().join("placed");

        let result = sync_skill(&src, &tgt, SyncMode::Place).unwrap();
        assert!(matches!(result, SyncMode::Place));
        assert!(tgt.is_symlink());
        let resolved = std::fs::read_link(&tgt).unwrap();
        // The symlink destination is the source directory.
        assert_eq!(resolved, src);
    }

    #[cfg(unix)]
    #[test]
    fn is_target_current_place_detects_correct_symlink() {
        let tmp = tempdir().unwrap();
        let src = tmp.path().join("source-dir");
        fs::create_dir_all(&src).unwrap();
        let tgt = tmp.path().join("placed");
        ...
        assert!(is_target_current(&src, &tgt, SyncMode::Place, None, None));
    }

    #[cfg(unix)]
    #[test]
    fn is_target_current_place_false_when_symlink_absent() {
        let tmp = tempdir().unwrap();
        let src = tmp.path().join("source-dir");
        let tgt = tmp.path().join("placed"); // not created
        assert!(!is_target_current(&src, &tgt, SyncMode::Place, None, None));
    }
}
