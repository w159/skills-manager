#!/usr/bin/env python3
"""Install the codebase-brain skill's hooks into a settings.json (GATED).

The codebase-brain skill ships two hooks (under ../hooks/). This wires them into a
Claude Code settings file by MERGING — it never clobbers existing hooks, and it is
idempotent (re-running is a no-op once installed). Dry-run by default; only --apply
writes, and it backs the settings file up first.

Hooks:
  load   SessionStart   load_brain.py     (inject the repo's committed brain at startup)
  gate   Stop           validate_gate.py  (block unverified completion claims, once)
...
Usage:
  ...
  python3 install_hooks.py --apply                     # install both
  python3 install_hooks.py --select load --apply       # just the SessionStart loader
...
Stdlib only. (Same merge/backup/idempotency contract as the orchestrate installer.)
"""
...
HOOK_SPECS = {
    "load": ("SessionStart", None, "load_brain.py", {}),
    "gate": ("Stop", None, "validate_gate.py", {}),
}
...
def cmd_list(settings: dict) -> None:
    print("codebase-brain hook coverage:")
    for hid, (event, matcher, script, _e) in HOOK_SPECS.items():
        ...
        print(f"  {state:16} {hid:5} -> {event}{m}")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Install codebase-brain hooks (gated).")
    ap.add_argument(
    ...
    selected = args.select or list(HOOK_SPECS)
    ...
    for p in plan(settings, selected):
        ...
        print(f"  {tag:8} {p['id']:5} -> {p['event']}  ({command_for(p['script'])})")
